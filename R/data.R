...
#'   \item{erp_calc_check}{Verification check for ERP calculations}
#'   \item{oa_bench_mark_price_calc}{Calculated Olympic average benchmark price}
#'   \item{current_arcco_benchmark_price}{Current ARC-CO benchmark price for revenue calculations}
...
#'
#' **Future Program Years (Carry-Forward Convention):**
#' Rows for program years beyond the latest available FSA releases carry forward
#' the most recent observed values for slow-moving farm attributes and
#' pre-election assumptions: PLC yields, county base acres, ARC/PLC enrollment
#' elections (producers assumed to keep their existing elections), and the
#' irrigated vs. non-irrigated planting mix used in the irrigation adjustment.
#' Realized outcomes (planted, prevented, and failed acres; NASS/RMA observed
#' yields) are NOT carried forward. MYA prices for future years are projections.
#' Payment calculations for such years are therefore projections conditional on
#' these assumptions. The carry-forward is self-superseding: when actual FSA
#' releases for a year are added to the data-raw inputs and the build is rerun,
#' the real values replace the carried-forward ones automatically.
#'
#' **Program Years:**
#' Currently includes data for program years 2009-2026. Enrolled base acre data
#' is comprehensive for 2019 onward; the most recent program year may rely on
#' the carry-forward convention described above.
#'
...
#' County-level additional base acres under the 2026 OBBBA base-acre update
#'
#' Estimated ADDITIONAL ARC/PLC base acres allocated to each county-crop under
#' the One Big Beautiful Bill Act (OBBBA) base-acre update, effective the 2026
#' crop year. Values are the net new base acres on top of a county-crop's
#' existing base; they do not include existing base acreage. Provided for eight
#' policy-interpretation scenarios (\code{s1}-\code{s8}); \code{s1} is the main
#' scenario. Used by \code{\link{calc_arc_plc_payments}} when
#' \code{updated_base_acres = TRUE} to expand the enrolled base that 2026+
#' payments are computed on.
#'
#' The statutory allocation is a five-year (2019-2023) average of covered-
#' commodity plantings plus a noncovered adjustment (capped at 15\% of total
#' farm acres), less existing base as of 2024-09-30, prorated so the national
#' total does not exceed the 30 million acre cap. Snapshotted from the
#' \code{proj-base-vs-planted-acres} project; regenerate via
#' \code{./data-raw/fsaUpdatedBaseAcres/fsaUpdatedBaseAcres.R}.
#'
#' @format A data frame with four variables:
#' \describe{
#'   \item{fips}{numeric. Five-digit combined state+county FIPS code.}
#'   \item{crop}{character. Covered commodity (whole-crop names matching
#'     \code{fsaArcPlcData}; RMA sub-types such as chickpeas and rice are
#'     collapsed to the whole crop).}
#'   \item{scenario}{character. Policy-interpretation scenario, one of
#'     \code{"s1"}-\code{"s8"} (\code{"s1"} is the main scenario).}
#'   \item{additional_base_acres}{numeric. Estimated additional base acres for
#'     the county-crop under the 2026 update (acres).}
#' }
#'
